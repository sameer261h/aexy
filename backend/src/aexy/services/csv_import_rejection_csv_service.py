...
neutralized against spreadsheet-formula injection, matching the same
`=`/`+`/`-`/`@` convention CRM table export uses elsewhere in this
repository (that feature lives on a sibling branch not in this branch's
history, so the technique is reimplemented here rather than imported).
"""
