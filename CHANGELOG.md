...
- **Desktop companion (Aexy for macOS)**: the macOS app (renamed to **Aexy**, in `aexy-mac/`) is now a hybrid companion — web sign-in, native Today/Tasks, and an embedded web view. The web app gains a **chromeless `?embed=true` mode** (`AppShell` hides its own sidebar) so the desktop app's native sidebar is the sole navigation while the web provides full feature parity.
