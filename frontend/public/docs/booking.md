...
> The booking module shares its Azure App Registration with Aexy sign-in and the CRM Outlook integration. For the full setup — including the second redirect URI, `MICROSOFT_TENANT_ID`, and CRM scopes — see [Microsoft integration](./microsoft.md). The steps below cover the booking-only minimum.
