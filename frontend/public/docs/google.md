...
3. **Periodic Sync**: Temporal schedule `check-gmail-auto-sync` polls every 60 seconds for integrations due for a sync
...
3. **Periodic Sync**: Shares the same `check-gmail-auto-sync` Temporal schedule that drives mail sync
