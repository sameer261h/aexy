...
const STATUS_QUICK_ACTIONS: { status: TaskStatus; icon: React.ReactNode; label: string; color: string }[] = [
  { status: "backlog", icon: <Circle className="h-3 w-3" />, label: "Backlog", color: "text-muted-foreground hover:text-foreground" },
...
export function TaskCardPremium({
  ...
  return (
    <motion.div
      ...
      onClick={handleClick}
      className={cn(
        "group relative bg-muted/80 backdrop-blur-sm rounded-lg p-3 border transition-all duration-200",
        "hover:border-muted-foreground hover:shadow-lg hover:shadow-black/20",
        isSelected
    ...
    >
      ...
      {onSelect && (
        ...
        >
          ...
          >
            {isSelected && <CheckCircle className="h-3 w-3 text-foreground" />}
          </div>
        </div>
      )}
      ...
      <div className="flex items-start justify-between mb-2 mt-1">
        <div className="flex items-center gap-2 flex-wrap">
          {task.source_type && (
            <span className="text-muted-foreground">{SOURCE_ICONS[task.source_type]}</span>
          )}
          ...
          {showMenu && (
            <>
              ...
              <motion.div
                ...
                className="absolute right-0 top-full mt-1 w-40 bg-accent/95 backdrop-blur-xl rounded-lg shadow-xl z-20 py-1 border border-border"
              >
                {task.source_url && (
                  <a
      ...
      {task.labels && task.labels.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-2">
          {task.labels.slice(0, 3).map((label, i) => (
            <span
              key={i}
              className="text-[10px] px-1.5 py-0.5 bg-accent/50 text-foreground rounded"
            >
              {label}
            </span>
          ))}
              ...
              {showQuickStatus && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-accent/95 backdrop-blur-sm rounded-lg shadow-xl z-20 py-1 border border-border min-w-[120px]"
                  >
                    {STATUS_QUICK_ACTIONS.map(({ status, icon, label, color }) => (
                      <button
                        ...
                        className={cn(
                          ...
                          status === task.status ? "bg-muted/50" : "hover:bg-muted/50",
                          color
                        )}
                      >
                        {icon}
...
export function TaskCardSkeleton() {
  return (
    <div className="bg-muted/80 rounded-lg p-3 border border-border/50 animate-pulse">
      ...
      <div className="h-3 w-full bg-accent/50 rounded mb-2" />
      <div className="flex items-center justify-between mt-2">
        <div className="flex items-center gap-1.5">
