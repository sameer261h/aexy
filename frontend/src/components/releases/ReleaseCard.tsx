...
const STATUS_CONFIG: Record<ReleaseStatus, { label: string; color: string; bgColor: string; icon: React.ReactNode }> = {
  planning: { label: "Planning", color: "text-slate-400", bgColor: "bg-slate-500", icon: <Circle className="h-3 w-3" /> },
  ...
  cancelled: { label: "Cancelled", color: "text-slate-500", bgColor: "bg-slate-600", icon: <XCircle className="h-3 w-3" /> },
};
...
export function ReleaseCard({ release, onClick, onDelete, className }: ReleaseCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Title and Version */}
      <div className="mb-2">
        <h3 className="text-sm font-medium text-white">{release.name}</h3>
        {release.version && (
          <span className="text-xs font-mono text-slate-400">v{release.version}</span>
        )}
      </div>

      {/* Description */}
      {release.description && (
        <p className="text-xs text-slate-400 mb-3 line-clamp-2">{release.description}</p>
      )}
