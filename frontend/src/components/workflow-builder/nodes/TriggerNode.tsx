...
export const TriggerNode = memo(({ data, selected }: NodeProps<TriggerNodeType>) => {
  ...
  return (
    <div
      className={`
        pl-5 pr-4 py-3 rounded-xl shadow-lg min-w-[180px] relative
      ...
      `}
    >
      {/* Left-side notch — gives the trigger node a distinct silhouette
          from action / condition / wait nodes so users can scan a
          dense canvas and find the entry point instantly. Inherits
          the error tint when validation fails so the affordance
          doesn't fight the rest of the node's state. */}
      <div
        aria-hidden
        className={`absolute left-0 top-2 bottom-2 w-1 rounded-r-full ${
          data.hasError ? "bg-red-500" : "bg-emerald-500"
        }`}
      />
      ...
      <div className="flex items-center gap-3">
        <div className={`p-2 rounded-lg ${data.hasError ? "bg-red-500/30" : "bg-emerald-500/30"}`}>
          <Icon className={`h-5 w-5 ${data.hasError ? "text-red-500 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`} />
        </div>
        <div>
          <div className="flex items-center gap-1.5">
            <span
              className={`inline-flex items-center text-[10px] uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded ${
                data.hasError
            ...
            <span className={`text-[10px] uppercase tracking-wider font-medium ${data.hasError ? "text-red-500/70 dark:text-red-400/70" : "text-emerald-700/70 dark:text-emerald-400/70"}`}>
              Trigger
            </span>
          </div>
      ...
      {data.hasError && data.errorMessage && !isRunning && !isSuccess && !isFailed && (
        <div className="mt-2 text-xs text-red-600 dark:text-red-400 bg-red-500/10 px-2 py-1 rounded">
          {data.errorMessage}
        </div>
      )}
