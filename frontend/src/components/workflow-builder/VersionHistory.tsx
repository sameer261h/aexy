    ...
    <div className="fixed inset-y-0 right-0 w-[450px] bg-slate-800 border-l border-slate-700 shadow-xl z-40 flex flex-col">
      ...
      <div className="px-4 py-2 bg-slate-700/50 border-b border-slate-700">
        <div className="text-xs text-slate-400">
          Current version: <span className="text-white font-medium">v{currentVersion}</span>
        </div>
      </div>

      {/* Compare Mode Header */}
      {compareMode && (
        <div className="px-4 py-3 bg-blue-500/10 border-b border-slate-700">
          <div className="text-sm text-blue-300 mb-2">Select two versions to compare</div>
          <div className="flex items-center gap-2 text-sm">
            <span className="text-slate-400">Version A:</span>
            <span className="text-white">{compareVersionA ?? "Select..."}</span>
            <ArrowRight className="h-4 w-4 text-slate-500" />
            <span className="text-slate-400">Version B:</span>
            <span className="text-white">{compareVersionB ?? "Select..."}</span>
          </div>
        </div>
      )}
        ...
        {isLoading ? (
          ...
          <div className="p-4 space-y-4">
            <div className="bg-slate-700/50 rounded-lg p-3">
              <h3 className="text-white font-medium mb-2">Changes Summary</h3>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {diff.nodes.added.map((node) => (
                    ...
                    >
                      <span className="text-white">{node.label || node.id}</span>
            ...
            {(diff.edges.added.length > 0 || diff.edges.removed.length > 0) && (
              <div>
                <h4 className="text-slate-300 text-sm font-medium mb-2">Connection Changes</h4>
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    {!compareMode && <ChevronRight className="h-4 w-4 text-slate-600" />}
                  </div>
                </div>
              </button>
            ))}
        ...
        <div className="p-4 border-t border-slate-700 bg-slate-800/90">
          <div className="bg-slate-700/50 rounded-lg p-3 mb-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-white font-medium">Version {selectedVersion.version}</span>
              <span className="text-xs text-slate-500">{formatDate(selectedVersion.created_at)}</span>
            </div>
            <p className="text-sm text-slate-400">{selectedVersion.change_summary || "No description"}</p>
            <div className="flex items-center gap-3 mt-2 text-xs text-slate-500">
