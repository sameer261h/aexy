...
export default function EmailMarketingPage() {
  ...
  const getDomainStatusColor = (domain: { is_active: boolean; warming_status: string; health_score: number }) => {
    if (!domain.is_active) return "bg-slate-400";
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400">Please select a workspace to view email marketing.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-400 text-sm">Campaigns, templates, and analytics</p>
              </div>
            </div>
            <Link
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-slate-400 text-sm">Avg Open Rate</span>
            </div>
            {analyticsLoading ? (
              <Loader2 className="h-6 w-6 text-slate-500 animate-spin" />
            ) : (
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-slate-400 text-sm">Avg Click Rate</span>
            </div>
            {analyticsLoading ? (
              <Loader2 className="h-6 w-6 text-slate-500 animate-spin" />
            ) : (
              <p className="text-2xl font-bold text-white">{stats.avgClickRate.toFixed(1)}%</p>
            )}
          </div>
          <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5">
            <div className="flex items-center gap-3 mb-3">
        ...
        {/* Tabs */}
        <div className="flex items-center gap-1 mb-6 bg-slate-900/50 border border-slate-800 rounded-lg p-1 w-fit">
          {[
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {campaigns.map((campaign) => {
                      ...
                      return (
                        <tr key={campaign.id} className="hover:bg-slate-800/50 transition">
                          <td className="px-6 py-4">
                            <Link href={`/email-marketing/campaigns/${campaign.id}`} className="flex items-center gap-3 group">
                              ...
                              <span className="text-white font-medium group-hover:text-sky-400 transition">{campaign.name}</span>
                            </Link>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4 text-slate-300">{campaign.sent_count.toLocaleString()}</td>
                          ...
                          <td className="px-6 py-4">
                            <Link href={`/email-marketing/campaigns/${campaign.id}`} className="text-slate-400 hover:text-white transition">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Link
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition text-sm"
                >
                  <Plus className="h-4 w-4" />
                  New Template
                </Link>
            ...
            {templatesError ? (
              ...
              <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-12 text-center">
                ...
                <p className="text-slate-400 mb-4">Create reusable email templates for your campaigns.</p>
                <Link
                  href="/email-marketing/templates/new"
                  className="inline-flex items-center gap-2 px-4 py-2 bg-sky-500 hover:bg-sky-600 text-white rounded-lg transition"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {templates.map((template) => (
                  <Link
                    ...
                    className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition cursor-pointer group"
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className="p-2 bg-sky-500/20 rounded-lg">
                        <Palette className="h-5 w-5 text-sky-400" />
                      </div>
                      <span className="text-white font-medium group-hover:text-sky-400 transition">{template.name}</span>
                    </div>
                    <p className="text-slate-500 text-sm mb-2">{template.template_type}</p>
            ...
            {analyticsError ? (
              ...
              <>
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                  <h2 className="text-lg font-semibold text-white mb-4">Performance Overview (Last 30 Days)</h2>
                  {analyticsLoading ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      <div className="text-center p-4 bg-slate-800/50 rounded-lg">
                        <p className="text-2xl font-bold text-white">{analyticsOverview.total_sent.toLocaleString()}</p>
                      ...
                      <div className="text-center p-4 bg-slate-800/50 rounded-lg">
                        ...
                        <p className="text-slate-400 text-sm">Bounces</p>
                      </div>
                      <div className="text-center p-4 bg-slate-800/50 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                    <h3 className="text-white font-medium mb-4">Top Performing Campaigns</h3>
                    {topCampaigns?.campaigns && topCampaigns.campaigns.length > 0 ? (
                      <div className="space-y-3">
                        {topCampaigns.campaigns.map((campaign) => (
                          <div key={campaign.id} className="flex items-center justify-between">
                            <span className="text-slate-400 truncate">{campaign.name}</span>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-white font-medium">Sending Domains</h3>
                  </div>
                  <button
                ...
                {domainsError ? (
                  ...
                  <p className="text-slate-500 text-sm">No sending domains configured</p>
                ) : (
                  <div className="space-y-3">
                    {domains.map((domain) => (
                      <div key={domain.id} className="flex items-center justify-between p-3 bg-slate-800/50 rounded-lg">
                        <div className="flex items-center gap-3">
              ...
              <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="text-white font-medium">Email Providers</h3>
                </div>
                {providersError ? (
                  ...
                  <p className="text-slate-500 text-sm">No email providers configured</p>
                ) : (
                  <div className="space-y-3">
                    {providers.map((provider) => (
                      <div key={provider.id} className="flex items-center justify-between p-3 bg-slate-800/50 rounded-lg">
                        <span className="text-white">{provider.name}</span>
                        <span className={`text-sm ${provider.is_active ? "text-emerald-400" : "text-slate-400"}`}>
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-white font-medium">Subscriber Overview</h3>
              </div>
              {analyticsLoading ? (
                <div className="flex justify-center py-4">
                  <Loader2 className="h-6 w-6 text-slate-500 animate-spin" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="text-center p-4 bg-slate-800/50 rounded-lg">
                    <p className="text-2xl font-bold text-white">{analyticsOverview?.total_unsubscribes || 0}</p>
                    <p className="text-slate-400 text-sm">Unsubscribed (30d)</p>
                  </div>
                  <div className="text-center p-4 bg-slate-800/50 rounded-lg">
