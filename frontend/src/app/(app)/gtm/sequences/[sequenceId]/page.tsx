...
const STATUS_STYLES: Record<string, string> = {
  draft: "bg-zinc-500/20 text-muted-foreground",
...
const ENROLLMENT_STATUS: Record<string, string> = {
  ...
  unsubscribed: "bg-zinc-500/20 text-muted-foreground",
  exited: "bg-zinc-500/20 text-muted-foreground",
...
function StepCard({
  ...
  return (
    <div className="relative">
      ...
      <div className="bg-muted/50 border border-border rounded-xl p-4 hover:border-border transition-colors">
              ...
              step.action !== "linkedin_view" && (
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-muted/50 border border-border rounded-lg px-2 py-1.5 text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:ring-1 focus:ring-indigo-500/50 resize-none"
                  />
                </div>
              )}
          </div>
  ...
  return (
    <div className="space-y-6 p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-foreground">{sequence.name}</h1>
              <span
        ...
        <div className="flex items-center gap-2">
          {sequence.status === "active" && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-muted/50 text-foreground hover:bg-muted rounded-lg text-sm transition-colors"
            >
      ...
      <div className="grid grid-cols-5 gap-4">
        {[
          { label: "Enrolled", value: sequence.enrolled_count, color: "text-foreground" },
        ...
        {(["steps", "enrollments", "analytics"] as const).map((tab) => (
          ...
          >
            ...
            {tab === "enrollments" && enrollmentTotal > 0 && (
              <span className="ml-1.5 text-xs text-muted-foreground">({enrollmentTotal})</span>
            )}
          </button>
        ))}
      </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left text-xs text-muted-foreground font-medium px-4 py-3">Contact</th>
                    <th className="text-left text-xs text-muted-foreground font-medium px-4 py-3">Status</th>
                    <th className="text-left text-xs text-muted-foreground font-medium px-4 py-3">Step</th>
                    <th className="text-left text-xs text-muted-foreground font-medium px-4 py-3">Enrolled</th>
                  ...
                  {enrollments.map((enrollment: OutreachEnrollment) => (
                    <tr key={enrollment.id} className="border-b border-border/50 hover:bg-muted/50">
                      <td className="px-4 py-3">
                        <div className="text-sm text-foreground">{enrollment.contact_name || enrollment.email}</div>
          ...
          {analytics ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-muted/50 border border-border rounded-xl p-4 text-center">
                <div className="text-2xl font-semibold text-foreground">{analytics.total_enrolled}</div>
            ...
            <div className="bg-muted/50 border border-border rounded-xl p-8 text-center">
              <p className="text-sm text-muted-foreground">Analytics will appear once contacts are enrolled.</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-background border border-border rounded-xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-foreground">Enroll Contact</h2>
              <button
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">CRM Record ID *</label>
                <input
                  type="text"
