/**
 * Focused coverage proving RecordHighlights' field-value rendering split:
...
const emailAttr = {
  id: "attr-email", slug: "email", name: "Email", attribute_type: "email", is_system: false,
} as CRMAttribute;
...
const record = {
  ...
  values: { email: "alice@example.com", primary_company: "company-id-1" },
...
describe("RecordHighlights field rendering", () => {
  it("renders a record_reference highlight via RelationshipFieldValue, not FieldRenderer", () => {
    fieldRendererMock.mockClear();
    render(
      <RecordHighlights
        record={record}
        attributes={[referenceAttr]}
        highlightAttributes={["primary_company"]}
  ...
  it("still renders non-reference attributes via the shared FieldRenderer, unchanged", () => {
    ...
    render(
      <RecordHighlights
        ...
        highlightAttributes={["email"]}
      />
    );
    expect(fieldRendererMock).toHaveBeenCalledWith(
      expect.objectContaining({ attribute: emailAttr, value: "alice@example.com", surface: "highlights" })
    );
  });
});
